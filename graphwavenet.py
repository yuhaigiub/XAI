...
class GraphWaveNet(nn.Module):
    def __init__(self,
        ...
        self.e1 = nn.Parameter(torch.randn(
            num_nodes, adptive_embeddings), requires_grad=True)
        self.e2 = nn.Parameter(torch.randn(
            adptive_embeddings, num_nodes), requires_grad=True)
        ...
        for d in dilations:
            ...
            self.gcn.append(GCNConv(in_channels=dilation_channels,
                                    out_channels=residual_channels))
    ...
    def __batch_timesteps__(self, x, edge_index, edge_weight=None):
        ...
        dataset = [Data(x=_x, edge_index=e_i, edge_attr=e_w) for _x, e_i, e_w in zip(x, edge_index, edge_weight)
                   ]
