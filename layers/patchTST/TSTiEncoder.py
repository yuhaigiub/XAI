...
from layers.patchTST.TSTEncoder import TSTEncoder
...
class TSTiEncoder(nn.Module):
    def __init__(self,
        ...
        self.W_P = nn.Linear(patch_len, d_model, device=device)
        
        # positional_encoding("zeros")
        W_pos = torch.empty((patch_num, d_model), device=device)
        nn.init.uniform_(W_pos, -0.02, 0.02).to(device)
        self.W_pos = nn.Parameter(W_pos, requires_grad=True)
        
        self.dropout = nn.Dropout(dropout)
        self.encoder = TSTEncoder(device, d_model, n_heads)
