...
class GraphWaveNet(nn.Module):
    def __init__(self,
        ...
        self.e1 = nn.Parameter(torch.randn(num_nodes, adptive_embeddings), requires_grad=isTraining)
        self.e2 = nn.Parameter(torch.randn(adptive_embeddings, num_nodes), requires_grad=isTraining)
    ...
    def forward(self, x, edge_index, edge_weight=None):
        ...
        for k in range(self.num_dilations):
            ...
            # the GCN
            data = self.__batch_timesteps__(g,
                                            edge_index,
            ...
            gcn_out = self.gcn[k](data.x, data.edge_index, edge_weight=torch.flatten(data.edge_attr))
            gcn_out = gcn_out.reshape(*batch_size, timesteps, -1, self.gcn[k].out_channels)
            
            gcn_out_adp = self.gcn_adp[k](g, adp)
            gcn_out_adp = gcn_out_adp.reshape(*batch_size, timesteps, -1, self.gcn[k].out_channels)
        ...
        if is_batched:
            ...
            x = x.reshape(self.out_timesteps,
                          self.out_channels,
                          self.num_nodes).transpose(-1, -2)
        return x
    ...
    def __batch_timesteps__(self, x, edge_index, edge_weight=None):
        ...
        for _x, e_i, e_w in zip(x, edge_index, edge_weight):
            dataset.append(Data(x=_x, edge_index=e_i, edge_attr=e_w))
